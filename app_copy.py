...
def llm_believable_test():
    if request.method == "POST":
        ...
        if len(request.form.get("prompt_input")) > 0:
            prompt_value = request.form.get("prompt_input")
        ...
        # 返回结果页面
        return render_template("llm_believable_test.html", 
                               result=result)
...
# @app.route('/llmindex', methods=["GET", "POST"])  # llm越狱攻击
# def llmindex():
#     if request.method == "POST":
#         # 获取用户选择的数据
#         module_value = request.form.get("module_choose")
...
#         if len(request.form.get("prompt_input")) > 0:
#             prompt_value = request.form.get("prompt_input")
#         template_value = request.form.get("template_choose")
#         if len(request.form.get("template_input")) > 0:
#             template_value = request.form.get("template_input")
...
#         result['type_template'] = request.form.get("template_input")
#         # 返回结果页面
#         return render_template("llmindex.html", 
#                                result=result)
